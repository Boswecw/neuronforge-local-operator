...
| Live adapter proof (pre-G-10) | Adapter, `nlo-graph verify-live`, and the opt-in integration test are committed; **pending operator run**. On PEP 668 hosts (Ubuntu 24.04) install into the repo venv per `requirements.txt` convention: `python3 -m venv .venv && source .venv/bin/activate && pip install -r requirements-dev.txt -r requirements-graphiti.txt` (the graph scripts auto-prefer `.venv` afterwards). Then: `bash scripts/graph/graph-up.sh`, `scripts/graph/nlo-graph rebuild --prove`, `scripts/graph/nlo-graph verify-live` (equivalently `NLO_GRAPH_LIVE_TEST=1 python3 -m pytest tests/experiment_memory/test_live_backend.py -q`). Pass = backend read-back fingerprint equals the projection report fingerprint and all five golden evidence queries match from the backend. Record the result here, then score Graphiti in the G-09 doc |
