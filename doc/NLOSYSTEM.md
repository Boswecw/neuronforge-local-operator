...
NeuronForge Local Operator is documented here as a local-first operator control surface. This overview anchors the canonical folder shape; detailed contract and runtime facts remain in the authored chapters.
...
## Status
Implemented through slice G-09; the pinned backend is verified live on operator hardware (2026-06-10: healthy, loopback-only); the live Graphiti adapter round-trip proof is operator-confirmed against real Neo4j 5.26.0 (provenance-equal read-back, five golden queries matching from the backend). Every gate ahead of G-10 (keep/revise/remove) is closed. Governing plan set: `docs/plans/graphiti/`.
...
Dependency truth for NeuronForge Local Operator must be sourced from repo manifests and executable verification. This chapter reserves the canonical dependency section.
...
Operational truth for NeuronForge Local Operator must be kept in executable commands, verification notes, and handoff records. This chapter reserves the canonical operations section.
...
Appendices hold glossary, contract cross-references, and historical notes for NeuronForge Local Operator system documentation.
